...
typedef struct edge { //Represents a edge, holding the destination, pointers to EdgeData and the next edge
    int destination;
    EdgeData contents;
    struct edge *next;
} Edge;

typedef struct node { //Represents a node, contains pointers to NodeData, list of edges and the next node
    int id;
    NodeData contents;
    Edge *start, *finish;
    struct node *next;
